...
Each rule follows a consistent structure with metadata fields for `severity`, `confidence`, `false_positive`, and `tags`, making it easy to integrate with existing security pipelines.
---

Installation & Usage
Prerequisites
YARA ≥ 4.x installed on your system
```bash
# macOS
brew install yara
...
```bash
   ...
   ```
Create your rule file in the appropriate category folder, following the naming convention `<platform>.yar`.
Follow the rule format described in the Rule Structure section above. Every rule must include all metadata fields.
Test your rules against real samples (redacted) and document the false positive rate in the `false_positive` meta field.
Open a Pull Request with a clear description of what is detected and a reference to the official documentation for the credential type.
---
